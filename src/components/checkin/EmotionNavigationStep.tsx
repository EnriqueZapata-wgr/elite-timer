 ...
 *
 * OLA5 pieza 2 — ESTO ERA UNA RUTA HERMANA Y ES UN PASO.
 * Solo se llegaba aquí desde el check-in: era el paso 3 de un flujo disfrazado
 * de pantalla. Ahora es una sub-máquina que /checkin monta tras la invitación
 * (y tras la rama de crisis). Al vivir dentro de la pantalla que la abrió, el
 * estado sobrevive al viaje a /breathing o /meditation sin trucos: la pantalla
 * sigue montada bajo la herramienta y el useFocusEffect la reencuentra viva.
 * `onClose` reemplaza al router.back() de la ruta: quien la montó decide a
 * dónde se vuelve.
 */
...
import { saveNavDraft, clearNavDraft } from '@/src/services/checkin-nav-draft';
...
type Props = {
  /** La emoción de origen: la que se acaba de registrar en el check-in. */
  emotionId: string;
  /** A dónde vuelve el paso (el check-in decide: cierre, plano o salir). */
  onClose: () => void;
  /**
   * Rescate del borrador (OLA5 pieza 2): la persona salió a una herramienta y
   * el proceso murió. Se entra directo al re-check-in con su cadena.
   */
  resumeChainIds?: string[];
};

export function EmotionNavigationStep({ emotionId, onClose, resumeChainIds }: Props) {
  ...
  const resumed = !!resumeChainIds?.length;
  // Rescatado, la persona ya cruzó la frase y el movimiento: entra al mapa.
  const [phase, setPhase] = useState<Phase>(resumed ? 'map' : 'frame');
  const [moveIndex, setMoveIndex] = useState(0);
  const [subStep, setSubStep] = useState<SubStep>(resumed ? 'tools' : 'question');
  ...
  const pendingRecheck = useRef<{ chainIds: string[] } | null>(
    resumed ? { chainIds: resumeChainIds! } : null,
  );
  const [recheck, setRecheck] = useState<'offer' | 'saved' | null>(resumed ? 'offer' : null);
  ...
  if (!plan || !origin) {
    ...
    return (
      <Screen themed>
        ...
        <PillarHeader pillar="mind" title="Navegar" onBack={onClose} />
  ...
  if (plan.crisis) {
    return (
      <Screen themed>
        <StatusBar style={kind === 'light' ? 'dark' : 'light'} />
        <PillarHeader pillar="mind" title="Acompañamiento" onBack={onClose} />
        <View style={styles.crisisWrap}>
          ...
          <Animated.View entering={FadeInDown.delay(150).duration(400)} style={[styles.crisisCard, cardSurf]}>
            ...
            <Pressable onPress={onClose} style={styles.stayLink}>
  ...
  const openTool = (tool: RegulationTool) => {
    ...
    if (move) {
      pendingRecheck.current = { chainIds: move.chainIds };
      // OLA5: seguro contra muerte del proceso mientras la persona respira.
      // El ref cubre el caso normal; esto cubre que Android mate la app.
      saveNavDraft({ emotionId: origin.id, chainIds: move.chainIds });
    }
  ...
  const dismissRecheck = () => {
    ...
    clearNavDraft();
    setRecheck(null);
    // Rescatado del borrador no hay flujo debajo al que volver: se cierra.
    if (resumed) onClose();
  };
    ...
    <Screen themed>
      ...
      {recheck === null && move && subStep === 'question' && (
        <Animated.View entering={SlideInDown.duration(280)} style={[styles.card, cardSurf]}>
          ...
          <View style={styles.cardActions}>
            <Pressable onPress={() => { haptic.light(); onClose(); }} style={styles.stayLink}>
