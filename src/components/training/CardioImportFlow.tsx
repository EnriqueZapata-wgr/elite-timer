/**
 * CardioImportFlow — la fase de importación de /log-cardio (Ola 2 Fitness
 * PR2, ANEXO_B_FITNESS §3; ex pantalla /cardio-import, MB-3.6 Bloque 3.2).
 * Importa entrenamientos desde la plataforma de salud del sistema
 * (Health Connect / HealthKit).
 *
 * Las 7 fases se conservan ÍNTEGRAS (cargando / consentimiento /
 * no_disponible / permiso_manual / leyendo / lista / resultado),
 * consentimiento primero: se explica qué se lee y para qué antes de pedir
 * el permiso del sistema. Solo lectura, solo entrenamientos (minimización
 * de datos). Import manual como default; auto-sync es opt-in. Estados
 * vacíos honestos: sin app conectada o binario sin el módulo, se dice tal
 * cual — sin culpar al usuario y sin prometer lo que no hay.
 *
 * `onManual` regresa al registro manual de /log-cardio (antes era un
 * router.replace entre pantallas; hoy es la misma pantalla).
 */
...
export function CardioImportFlow({ onManual }: { onManual: () => void }) {
        ...
        {fase === 'no_disponible' && plataforma && (
          <Animated.View entering={FadeInDown.duration(300)} style={[s.card, cardTk]}>
            ...
            <GradientCTA label="Registrar manual" variant="quiet" icon="add-circle-outline"
              onPress={() => { haptic.light(); onManual(); }} style={{ marginTop: Spacing.sm }} />
          </Animated.View>
        )}
