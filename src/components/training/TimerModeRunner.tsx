/**
 * TimerModeRunner — modo timer del runner unificado /session (Ola 2 Fitness
 * PR1, ANEXO_B_FITNESS §2). Es la interfaz de /execution absorbida como
 * componente: motor useRoutineEngine + flattenRoutine, CircularTimer con
 * anillo/rojo final/aura, píldoras de rondas anidadas, play/pause/skip/
 * restart step/restart rutina, preview SIGUIENTE + DESPUÉS, stats en vivo
 * con mini-barra, cierre de timer (total, % trabajo/descanso, saltados,
 * REPETIR), analytics WORKOUT_* mode timer, crédito conditioning
 * (cardio_sessions 'other' + electrón tras insert confirmado) y
 * useRegisterOwnNav (dibuja su propia flecha).
 *
 * /execution sigue vivo como fallback hasta el PR3; este componente es la
 * copia canónica que corre dentro de /session cuando la rutina es de puro
 * tiempo (el árbitro es routineUsesClipRunner: el contenido decide).
 */
...
// === RUNNER DE MODO TIMER ===

export function TimerModeRunner({ routine }: { routine: EngineRoutine }) {
  // 19.1: este modo dibuja su propia flecha — registra nav propia y la
